			...
			start() {
				...
				function positionLoop() {
					...
					*/
					...
					if (positions && document.getElementById('glass1').style != null) {
						for (var p = 0; p < 71; p++) {
							ploc = positions[27];
							ploca = positions[32];
							ploco = positions[33];
							...
							plocw = positions[14];
							if (p === 70) {
								document.getElementById('glass1').style.top = ploco[1].toFixed(2) - ((plocw[0] - plocv[0]) / 2).toFixed(2) + 0 + "px";
								document.getElementById('glass1').style.left = ploco[0].toFixed(2) - ((plocw[0] - plocv[0]) / 2).toFixed(2) + "px";
								document.getElementById('glass1').setAttribute('width', (plocw[0] - plocv[0]).toFixed(2) + "px");
							}
						}
					}
					if (positions && document.getElementById('glass1').style != null) {
