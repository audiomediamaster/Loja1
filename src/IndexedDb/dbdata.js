...
(function showIndexedDbSize() {
 ...
 if (!'indexedDB' in window != null) {
  function openDatabase() {
    return new Promise(function(resolve, reject) {
      //prompt for DB name
      var dbname =   'firebaseLocalStorageDb'; //prompt('Please enter your Database Name', '');
...
/* function readAllData(st) {
  return openDatabase().then(function(db) {
      var tx = db.transaction(st, IDBTransaction.READ_ONLY);
      var store = tx.objectStore(st);
      return store.getAll();
    });
}
	readAllData('firebaseLocalStorage').then(data => {
